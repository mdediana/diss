...
armazenamento distribuído em uma WAN. Um estudo completo consideraria também
outros atributos de qualidade como disponibilidade e manutenibilidade, mas que
não são feitos aqui por limitações de escopo. Estudos futuros podem contemplar
esses outros atributos de qualidade.
...
\ref{sec:sistema_de_armazenamento}). Como o objetivo desse estudo visa comparar
...
memória ou disco) \item Velocidade do disco (para armazenamento em disco) \item
Quantidade de nós do sistema \item Tamanho do banco de dados \item Algoritmo de
particionamento % qtde de partições p/ consist hashing \item Configuração de
replicação -- $N$, $R$ e $W$ (para consistência em momento indeterminado) \item
Limiar para migração (para consistência na linha do tempo) \item Interface de
acesso \item Nível de log \end{itemize}
...
Latência \item Variação da latência \item Distribuição da latência \item Taxa de
perda de pacotes \item Taxa de duplicação de pacotes \item Taxa de corrupção de
pacotes \item Proporção de pacotes fora de ordem \item Algoritmo de
congestionamento (para TCP) \item Relação entre banda e latência (Produto
Banda-Latência -- BDP) (para TCP) \end{itemize}
...
Popularidade dos objetos acessados \item Taxa de chegada de requisições
\end{itemize}

\subsection{Seleção de fatores a serem estudados} \label{sec:selecao_de_fatores}
