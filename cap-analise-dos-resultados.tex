...
O nível de confiança do estudo relativo ao tamanho da amostra está descrito na
Seção~\ref{sec:nivel_de_confianca_exatidao_tamanho_da_amostra_e_replicacoes}, a
Seção~\ref{sec:analise_dos_tempos_de_resposta} mostra a análise dos tempos de
resposta, a Seção~\ref{sec:analise_dos_conflitos}, a análise dos conflitos e a
Seção~\ref{sec:analise_das_migracoes}, a análise das migrações. A
...
O tamanho da amostra de um experimento é dado por \cite{Jain1991}:
...
média da mesma amostra. O erro padrão é dado por:
...
A exatidão indica qual a distância entre a média observada e a média da
...
experimentos. Com precisão de 1\%, apenas os experimentos para \emph{lt\_rec},
localidade de 50\% e latência de rede 300 ms não tinham tamanho da amostra
...
mas tinha 76137. Se considerado o mesmo nível de confiança, mas exatidão de 2\%,
...
causa seria variação no desempenho na LAN do Grid'5000 em diferentes momentos.
...
representassem o comportamento do sistema no estado estacionário.
...
boxplot para latência de rede de 200~ms está na Figura
\ref{fig:boxplot_dos_tempos_de_resposta_para_latencia_de_rede_de_200_ms}. Os casos
em que a caixa não aparece indicam que todas as requisições delimitadas pelos
bigodes eram locais.
...
\caption{Boxplot dos tempos de resposta para latência de rede de 200 ms.}
...
apresentando requisições remotas (valores atípicos no gráfico).
...
Os gráficos das FDAs para latência de rede de 200 ms estão na
Figura~\ref{fig:fda_dos_tempos_de_resposta_para_latencia_de_rede_de_200_ms}.
Eles mostram a proporção entre requisições locais e remotas, e confirmam que a
localidade define essa proporção para \emph{lt\_qqer} e \emph{lt\_rec}.

\begin{figure}[p] \centering

\includegraphics[width=1\textwidth]{ecdf200.png}

\caption[FDAs dos tempos de resposta para latência de rede de 200 ms.]{FDAs dos
tempos de resposta para latência de rede de 200 ms. A linha vertical indica a
latência da rede (200 ms).}
\label{fig:fda_dos_tempos_de_resposta_para_latencia_de_rede_de_200_ms}
\end{figure}

Os ganhos para requisições remotas da consistência na linha do tempo com relação
à consistência em momento indeterminado com $W = 2$ e latência de rede de 200 ms
...
tempos de resposta para os modos \emph{lt\_qqer} e lt\_rec com relação a
\emph{ind2} para latência de 200 ms. O sinal negativo indica perdas.}
\label{tab:ganhos_relativos_das_requisicoes_remotas} \end{center} \end{table}

O valor 1 significa que o desempenho é igual, valores menores que 1 indicam que
\emph{ind2} apresenta melhor desempenho e valores maiores que 1 indicam que o
modo comparado a \emph{ind2} apresenta melhor desempenho. Com relação ao
percentil 75 e localidade de 50\%, os ganhos são altos pois as requisições são
locais para \emph{lt\_qqer} e \emph{lt\_rec} e são remotas para \emph{ind2}.
...
latência de 60\%. Para \emph{jitter} de 0\%, os conflitos são da ordem de 0,1\%.
...
remotas demoram centenas de milissegundos. Quando não há \emph{jitter}, duas
requisições saindo de um CPD chegam no outro CPD na ordem em que foram emitidas.
Mas quando há \emph{jitter}, é possível que a segunda chegue antes da primeira.
...
45\%, e com exceção da variação de 5\%, que apresentou 17\% de conflitos, todos
...
uma WAN, pois muitas vezes ela vai acionar o mecanismo de resolução de conflitos
desnecessariamente, resultando em duas escritas consecutivas na réplica remota.
