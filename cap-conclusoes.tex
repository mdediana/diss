...
provavelmente seria diferente devido a interação entre disco, cache de disco e
memória.
...
que a imagem usada neste estudo tem poucos softwares instalados e está
documentada (ver Subseção \ref{sec:ambiente}). Além disso, alguns poucos scripts
para execução do estudo precisariam ser alterados.
...
\paragraph{Infraestrutura de experimentos}A segunda contribuição é a
infraestrutura de experimentos que pode ser reaproveitada para outros estudos do
gênero, em especial se o estudo for feito no Grid5000 e usar o Riak.
...
tratáveis metodologicamente. Um exemplo disso a experiência do autor com a
operação no Grid5000 influenciar a decisão sobre o tamanho do sistema, uma
limitação que não foi mensurada com exatidão, mas sim percebida pelo trabalho
diário na plataforma (ver
...
executados, contra os 64 experimentos e 6,5h da segunda versão do estudo, que
redução que aconteceu pelo uso de experimentos 2\textsuperscript{k} (ver
Seção~\ref{sec:selecao_dos_fatores}).

\paragraph{Reprodutibilidade}Além de importante para a qualidade da pesquisa, o
a reprodutibilidade do estudo facilitou executá-lo muitas vezes para comparações
e análises. Isso foi consequência de ter o processo automatizado e, portanto,
...
Erlang, bash, Ruby e R -- entender o funcionamento do Linux foi importante por
dois pontos de vista. O primeiro foi a forma como ele gerencia o hardware, como
aconteceu na discussão sobre a relação entre disco, cache de disco e memória,
por exemplo (ver Subseção~\ref{sec:aglomerado_e_mecanismo_de_armazenamento}). O
segundo foram suas ferramentas para tarefas de administração de sistema como
top, vmstat, iostat, bwm-ng, etc.
...
\item Seminário ``NoSQL: Vantagens, desvantagens e compromissos''\footnote{Slides disponíveis em
...
\item Apresentação ``NoSQL: Perdas e Ganhos''\footnote{Slides disponíveis em \url{http://www.slideshare.net/mdediana/no-sql-perdaseganhos}.} na The
...
\item Relatório de desempenho da DHT chamada BlobSeer, produzido em estágio em
estágio de um mês no INRIA em Rennes, França, sobre análise de desempenho desse
sistema.
...
Um dos trabalhos futuros mais interessantes seria a validação ou refutação das conclusões deste
estudo em uma outra plataforma, de preferência uma que seja usada por aplicações
web de produção, como o AWS, por exemplo. Os resultados poderiam
ser diferentes por influência da virtualização, devido a latência da rede não
variar necessariamente seguindo uma distribuição normal, dentre outros fatores.

Outro tipo de trabalho poderia considerar diferentes discos, incluindo discos de
estado sólido (\emph{Solid State Drives} -- SSDs), cujo uso em centros de dados
vem crescendo. Seria interessante analisar e comparar a influência de cada um
dos dois tipos de E/S -- rede e disco -- no desempenho do sistema.
