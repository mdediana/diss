...
%por objetivo definir quais características do sistema, do ambiente e da carga
%de trabalho seriam consideradas e quais seriam desconsideradas no estudo e está
...
armazenamento e do \emph{benchmark} usados, bem como do ambiente em que operaram nos
experimentos, se encontram na Seção~\ref{sec:aspectos_tecnicos}. Por fim, os
...
Eeste trabalho usa como principal referência metodológica o livro \emph{The Art of Computer Systems Performance Analysis} (A
Arte da Análise de Desempenho de Sistemas de Computação) \cite{Jain1991}, de Raj
Jain. Esse livro trata da análise de desempenho de sistemas de software
e/ou hardware, e apresenta conceitos de estudos experimentais e o ferramental
estatístico necessário nesse tipo de análise. Além disso, ele propõe uma
abordagem sistemática para fazer um estudo experimental. Neste trabalho seguimos
os passos dessa abordagem, com exceção do levantamento de parâmetros e a seleção
de fatores, que ocorreram após a definição da técnica de avaliação.  Isso foi
feito pois essa foi a fase maior complexidade do estudo, inclusive estando
descritos em um capítulo à parte (Capítulo~\ref{cap:parametros_e_fatores}) por
esse motivo.
...
considerada uma meta principal pois o \emph{benchmark} usa uma taxa de requisições
fixada em vez de usar a máxima possível, de forma a diminuir possíveis
...
é definida pelo \emph{benchmark}, descrito na Subseção~\ref{sec:benchmark}. O ambiente
...
Por último, outras alterações menores no riak\_kv foram o tratamento de parâmetros extras na interface
...
O \emph{benchmark} usado foi o Basho Bench \cite{Basho}, específico para o Riak. Ele
provê configurações para quantidade de clientes concorrentes (threads), relação
leitura/escrita, popularidade dos objetos acessados, entre outras. O \emph{benchmark}
...
evitar gargalos no \emph{benchmark}, isso era importante para a implementação de
localidade. Além disso, foi necessária a implementação de um pequeno programa em Erlang
(basho\_bench\_dist) para a
consolidação dos dados obtidos pelas diversas instâncias do Basho Bench. Parte
dessa consolidação envolveu a fusão de histogramas usados pelo Basho Bench que
por sua vez usavam o projeto basho\_stats, que também precisou ser modificado.

Outro \emph{benchmark} considerado foi o
YCSB\footnote{\url{git://github.com/brianfrankcooper/YCSB.git}} \cite{Cooper2010}.  Apesar de possuir
...
modificações do basho\_stats em \url{https://github.com/mdediana/basho\_stats.git}.
...
Pesquisadores com acesso ao
Grid5000 podem acessar essa imagem, que se encontra no sítio Sophia em
\emph{/home/madediana/images/squeeze-x64-riak.tgz}.
...
comunicam através de WANs \cite{Jones2006,ESNet2012}. Durante os experimentos, sempre que os parâmetros da
...
armazenamento, configuração e execução do \emph{benchmark} e coleta dos resultados. Os
...
reservados entre nós de Riak e instâncias do \emph{benchmark}, a partir das quantidades
...
da LAN. Após isso, a execução do \emph{benchmark} propriamente dita ocorre. Por fim,
...
São alguns os tipos de experimentos usados em um estudo
...
2\textsuperscript{k} experimentos. O desempenho em função dos fatores é expresso por um modelo de regressão não-linear da forma:
...
define-se uma relação entre os níveis dos fatores e os valores -1 ou 1.
...
Para um projeto de experimentos 2\textsuperscript{k}, a soma total dos quadrados é dada por
(para a derivação dessa equação a partir da anterior, ver \cite{Jain1991}):
