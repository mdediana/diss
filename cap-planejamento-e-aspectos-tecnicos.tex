...
exemplo, independente do tamanho da memória, se diz que CPU e memória não
interagem. Mas se ao mudar a CPU de A para B o desempenho aumenta em 10\% quando
o sistema tem 4 GB, mas e 30\% quando tem 8 GB, se diz que CPU e memória
interagem.
...
encontrados sistemas de armazenamento de software livre que implementem
consistência na linha do tempo, as soluções avaliadas foram aquelas que
implementam consistência em momento indeterminado. Os sistemas encontrados foram
Dynomite \cite{Dynomite}, Cassandra \cite{Lakshman2010}, Voldemort
...
inseridas e 222 removidas em 18 \emph{commits} nos seguintes 11 arquivos:
...
foram 106 linhas inseridas e 17 removidas em 4 \emph{commits} nos seguintes 3
arquivos:
...
Os testes foram executados manualmente. Os principais cenários testados (CD se
refere a centro de dados):

\begin{itemize}

\item Requisição de inserção a partir do CD1 ocorrendo no CD1

\item Requisição de inserção a partir do CD1 sendo redirecionada para o CD2

\item Verificação de que inserções resultavam em ao menos uma réplica em cada
centro de dados

\item Requisição de leitura/atualização a partir do CD1 sempre ocorrendo no CD1
para consistência em momento indeterminado (local)

\item Requisição de leitura de ``qualquer versão'' a partir do CD1 sempre
ocorrendo no CD1 para consistência na linha do tempo (local)

\item Requisição de leitura da ``versão mais recente''/atualização a partir do
CD1 sendo redirecionada para a réplica mestre no CD1 para consistência na linha
do tempo (local)

\item Requisição de leitura da ``versão mais recente''/atualização a partir do
CD1 sendo redirecionada para a réplica mestre no CD2 para consistência na linha
do tempo (remota)

\item \emph{LM} requisições de atualização a partir do CD1 de um objeto no CD2
resultando em migração da réplica mestre para o CD2, onde \emph{LM} é o limiar
de migração
...
Por fim, outro indício de que implementação estava correta é que os resultados
apresentados no Capítulo~\ref{cap:analise_dos_resultados} estão de acordo com o
comportamento esperado de cada modelo de consistência.
...
A partir do momento em que o git passou a ser usado para controle de versão,
foram 90 linhas inseridas e 17 removidas em 16 \emph{commits} nos seguintes 3
arquivos do Basho Bench:
...
No basho\_stats, foram 77 linhas inseridas e 35 removidas em 2 \emph{commits}
nos seguintes 2 arquivos:
...
o mesmo centro de dados saíam inalteradas. Esse processo é executado pelo script
net (ver Subseção \ref{sec:execucao_e_analise_dos_experimentos}.
