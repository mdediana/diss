...
Consistência mais rígida implica em disponibilidade ou a tolerância a
...
linha do tempo de acordo com diferentes cargas de trabalho e latências de rede.
Por limitações de escopo, este trabalho não analisa a relação entre esses
modelos de consistência e disponibilidade, manutenibilidade ou qualquer outro
atributo de qualidade influenciado por eles.
...
cargas de trabalho (incluindo questões de localidade), a latência da rede, a
latência dos disco em cada nó, os algoritmos de resolução de conflitos, etc.
Assim, a adoção de simulação ou modelagem analítica sacrificaria o realismo e a
precisão deste trabalho. Daí a escolha de experimentação, pela qual é possível
analisar o comportamento de um sistema de armazenamento, resultado da interação
de vários dos fatores que afetam o desempenho de um modelo de consistência.

A seção \ref{sec:aspectos_tecnicos} descreve o ambiente dos experimentos, o
sistema de armazenamento no qual os diferentes modelos de consistência são
implementados e a aplicação que executa os testes. A seção
\ref{sec:projeto_dos_experimentos} trata do projeto dos experimentos.
...
características (incluindo arquitetura) e da carga de trabalho apresentada ao
sistema. O ambiente dos experimentos é definido pela plataforma escolhida para
executá-los, descrita na subseção \ref{sec:ambiente}. O sistema de armazenamento
usado como objeto de testes, no qual são implementados os modelos de
consistência, é descrito na subseção \ref{sec:sistema_de_armazenamento}. Por
fim, a carga sobre o sistema é definida pela aplicação de execução dos testes,
descrita na subseção \ref{sec:aplicacao_de_execucao_dos_testes}.

%% ------------------------------------------------------------------------- %%
\subsection{Ambiente} \label{sec:ambiente}

Os experimentos serão executados no Grid5000
\footnote{\url{http://www.grid5000.fr/}}, uma plataforma para criação, execução
e monitoramento de experimentos de sistemas paralelos e distribuídos. A
...
trabalho utilizou o Grid5000 por 4 meses em um estágio de mestrado pelo INRIA na
França. A experiência no ambiente adquirida nesse período pode ser reutilizada
neste trabalho, tornando o projeto e a execução dos experimentos mais
produtivos. O Amazon Web Services (AWS) \footnote{\url{http://aws.amazon.com/}}
também foi considerado, mas foi descartado por não ser possível ter controle
sobre a latência entre os centros de dados, o que dificultaria a
reprodutibilidade dos experimentos.

Os sítios do Grid5000 são conectados por redes de alta velocidade, com latências
da ordem de 20 ms, e portanto não caracterizam uma WAN. Por isso os experimentos
implementam uma latência artificial para simular uma WAN entre os centros de
dados. A vantagem sobre o uso de uma WAN real é que essa abordagem possibilita
maiores controle e reprodutibilidade dos experimentos.
...
modelo de consistência. Apesar de existirem experimentos que usem essa abordagem
...
aplicação será modificada, pois mais de uma instância dela será executada
simultaneamente nos experimentos, e portanto a geração dos acessos que
consideram localidade precisa ser coordenada entre elas.
