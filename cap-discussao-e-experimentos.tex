...
O experimento consiste em rodar o sistema dividido entre dois centros de dados
...
%TODO: Experimento x experimentos x campanhas x estudo x testes (considerações
%preliminares)
...
reserve exclusivamente para ele servidores por um determinado período de tempo.
Além de servidores, é possível reservar IPs para compor sub-redes, recurso
...
pela implantação de imagens nos servidores reservados pelo usuário.
...
experimento ter controle sobre esses valores para poder testar os sistema em
...
funcionais do experimento.

\subsubsection{Simulação de centros de dados}
\label{sec:simulacao_de_centros_de_dados}

Os servidores que compõem o sistema são divididos em dois conjuntos CD1 e CD2,
cada um representando um centro de dados. Além dos servidores, são reservadas
duas sub-redes SR1 e SR2. IPs de SR1 e SR2 são alocados para cada cada servidor
de CD1 e CD2, respectivamente. Ao final, cada servidor possui dois IPs, o seu IP
da rede do Grid5000 e um IP de uma das sub-redes.

Todos os servidores no Grid5000 possuem um nome registrado no DNS que aponta
para o seu IP da rede do Grid5000. Nesse experimento, são criados dois arquivos
/etc/hosts, um por centro de dados. Um deles é colocado em todos os servidores
de CD1 e o outro em todos os servidores de CD2. A partir dessa configuração, os
servidores do CD1 resolvem os nomes dos servidores do CD2 para os IPs de SR2 e
...
Para os experimentos, cada instância da aplicação de execução dos testes é
dedicada a um centro de dados. Essa configuração facilita a interpretação dos
dados e evita possíveis gargalos.

%% ------------------------------------------------------------------------- %%
\subsection{Imagem} \label{sec:imagem}

A imagem usada nessa pesquisa é um Debian Squeeze baseado em uma imagem
...
iperf\footnote{\url{http://iperf.sourceforge.net/}}. Existe uma
única imagem, a distinção entre servidores com Riak ou Basho Bench é feita
através dos scripts que gerenciam o experimento
(\ref{sec:infraestrutura_dos_experimentos}).
...
reserva de servidores, instalação da imagem, gerenciamento do Riak, configuração
