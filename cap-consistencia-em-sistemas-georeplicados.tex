...
Este trabalho usa o termo ``sistema de armazenamento'' para se referir de forma
...
de de sistemas de larga escala. Por fim, a
Seção~\ref{sec:trabalhos_relacionados} apresenta os trabalhos relacionados a
este.
...
dados em diferentes localizações geográficas \cite{DeCandia2007,Cooper2008} --
redes locais (\emph{Local area network} -- LANs) apresentam latências da ordem
centenas de microssegundos \cite{Rumble2011}. A segunda razão para replicar
dados é aumentar a vazão do sistema por meio do balanceamento de carga entre
diferentes servidores, possibilitando que o mesmo dado seja servido por mais de
um servidor simultaneamente. Finalmente, a terceira razão é o aumento da
disponibilidade do sistema, pois determinado dado continua sendo acessado pelos
clientes mesmo quando algumas das réplicas encontram-se indisponíveis.
...
Em casos mais extremos, a complexidade semântica dos conflitos impossibilita que
a resolução seja realizada pelo sistema de armazenamento -- por exemplo, caso um
mesmo objeto tenha sido atualizado simultaneamente com valores divergentes em
duas réplicas. Nesses casos, as respostas do sistema de armazenamento para
leituras feitas pela aplicação contêm os valores divergentes. Isso implica que,
para resolver os conflitos, os desenvolvedores precisam prever que eles podem
acontecer e preparar a aplicação para resolvê-los \cite{Davidson1985}.
...
geograficamente distribuído as latências entre diferentes centros de dados são
grandes. Garantir consistência mútua por meio de replicação pessimista nesse
contexto implica que toda operação apresentará um tempo de resposta relacionado
com a latência do enlace mais lento entre as réplicas, algo muitas vezes
...
objeto para o centro de dados no qual ele é escrito mais frequentemente.
...
de consistência escolhido por eles. O COPS usa consistência causal+, que é
semelhante à consistência causal com algumas garantia a mais, e implementa
