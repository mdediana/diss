...
de sistemas georeplicados, eles são necessários para um entendimento mais
profundo do projeto e desenvolvimento desse tipo de sistema.

A Seção~\ref{sec:sistemas_de_armazenamento} faz algumas considerações sobre a
nomenclatura usada para os sistemas tratados neste trabalho. A
...
de replicação e as vantagens e desvantagens de cada um. Um sistema pode
apresentar diferentes garantias de consistência dependendo da forma como replica
seus dados, a Seção~\ref{sec:modelos_de_consistencia_para_replicacao} descreve
...
mostra como diferentes modelos de consistência resultam em diferentes princípios
arquiteturais para sistemas de larga escala. Por fim, a
Seção~\ref{sec:trabalhos_relacionados} apresenta os trabalhos relacionados a
este.

%% ------------------------------------------------------------------------- %%
\section{Sistemas de Armazenamento} \label{sec:sistemas_de_armazenamento}

A área de bancos de dados tradicionalmente trata de bancos de dados que utilizam
o modelo relacional de dados, criado no início dos anos 70 \cite{Codd1970}. Essa
...
SGBDDs, como tabelas de espalhamento distribuídas (DHTs -- \emph{Distributed
hash table}) e sistemas de arquivos distribuídos. Esses sistemas lidam com
questões semelhantes, como replicação, por exemplo, o que faz com que muitas
técnicas surgidas em um contexto sejam aplicadas em outro. Considerando que
muitas vezes a distinção entre esses diferentes tipos de sistema não é clara,
sempre que possível o termo sistema de armazenamento é utilizado ao longo deste
trabalho para se referir a eles de forma genérica.
...
em um mesmo bastidor (\emph{rack}) \cite{Shvachko2010}, quanto para
longas distâncias, quando se replica dados entre centros de dados em diferentes
...
\cite{Tanenbaum2006}. Do ponto de vista do desenvolvedor, um modelo de
...
três possibilidades de leituras. A leitura da ``versão mais recente'' lê o último valor
...
enfrentados pelos desenvolvedores de sistemas de larga escala georeplicados e
