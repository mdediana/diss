...
\item Níveis: Valores dos fatores nos experimentos.

\item Interação entre fatores: Quando o desempenho varia sempre da mesma forma
ao se alterar o nível de um determinado fator, não há interação entre esse e os
demais fatores. Mas se a variação do desempenho acontece de acordo com o nível
...
ponto de vista do desempenho. Isso implica que o estudo não considera outros
...
outras condições podem ser estudadas em trabalho futuros.
...
de resposta, melhor o desempenho de um modelo de consistência. Como requisições
locais e remotas apresentam tempos de resposta bem diferentes, percentis em vez
de média são usados na maior parte das análises.
...
consistência, uma configuração de rede (latência, perda de pacotes, etc) e uma
...
São alguns os tipos de experimentos que podem ser usados em um estudo. A escolha
entre eles se dá principalmente pelo balanço entre recursos utilizados (tempo,
dinheiro, energia, etc) e quantidade de informação -- quando mais recursos
...
experimento que compõe o estudo. Existem dois tipos principais de projeto de
...
Este estudo usou um projeto de experimentos fatoriais fracionado para seleção de
fatores e níveis e um projeto de experimentos fatoriais completo para o estudo
final, ambos descritos no capítulo \ref{cap:projeto_dos_experimentos}. O
procedimento da seleção de fatores a partir de um projeto de experimento
fracionados 2\textsuperscript{k} estão descritos abaixo.
...
implementadas. Importante para a análise do desempenho com relação à
localidade,foi implementada a heurística do PNUTS em que a réplica mestre migra
para o centro de dados de onde vieram as últimas escritas.
...
sendoresolvidos para os IPs de SR1, o mesmo valendo entre hosts2 e SR2. Com
...
todas as requisições que saíam de um centro de dados para o outro usavam o IP
daquela sub-rede, enquanto as requisições para o mesmo centro de dados usavam o
