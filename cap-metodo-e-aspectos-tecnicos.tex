...
%% ------------------------------------------------------------------------- %%
\subsection{Scripts de execução e análise dos experimentos}
\label{sec:scripts_de_execucao_e_analise_dos_experimentos}

A automatização das etapas dos experimentos foi feita em dois conjuntos de
scripts. O primeiro (cmb) foi usado para gerenciamento e execução dos
experimentos -- reserva de nós, implantação da imagem, gerenciamento do sistema
de armazenamento, configuração e execução do benchmark e coleta dos resultados.
Os scripts se localizavam no frontend do Grid5000 e foram escritos em bash.  O
fluxo de trabalho de um estudo, dado por esses scripts, está descrito a seguir.
O segundo conjunto (cmb-local) foi usado na análise dos dados, se localizava no
computador do autor e era composto por scripts em bash, Ruby e R.
...
cada uma delas. O fluxo de trabalho em si também estava definido em um script,
que invocava os outros na sequência apresentada na Figura
\ref{fig:fluxo_de_trabalho_da_execucao_do_estudo}. As etapas estão descritas a
seguir, entre parênteses está o nome do script responsável por ela.
...
imagem nos nós};
    ...
    \node [block, below of=start] (load) {Carga do sistema};
...
experimento?};
    ...
    \node [decision, above of=loop, node distance=10em] (reconf) {Modo ou
localidade mudaram?};
...
\paragraph{Configuração dos nós (srvs)}Essa etapa consiste em separar os nós com
imagens implantadas entre nós de Riak e instâncias do benchmark, a partir das
quantidades de cada definidas para o estudo. Nesse momento, os nós de Riak
...
de configuração do Riak para cada nó. É nesse momento que o modelo de
consistência que o Riak vai usar é definido.

\paragraph{Inicialização do Riak (start)}Essa etapa corresponde à inicialização
das instâncias de Riak em cada nó e criação do aglomerado de Riak, quando as
partições do anel consistente são distribuídas entre as instâncias.

\paragraph{Carga do sistema (load)}Essa é uma das etapas mais longas e complexas
do fluxo de trabalho. A primeira parte da carga do sistema é popular o banco de
dados do Riak. Para consistência na linha do tempo existe uma segunda parte, que
é o aquecimento do sistema (detalhes dessa parte estão na subseção
\ref{sec:aquecimento}).

\paragraph{Configuração da WAN emulada}Nessa etapa as interfaces de rede dos nós
são configuradas com latência de rede, perda de pacotes, etc como descrito em
\ref{sec:rede}.

\paragraph{Execução de um experimento}Essa etapa começa pela aquisição de
informações dos nós, como as condições de memória e disco dos nós e testes de
ping entre nós. Após isso, a execução do benchmark propriamente dita ocorre. Por
