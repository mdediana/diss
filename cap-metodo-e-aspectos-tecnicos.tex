...
\begin{tikzpicture}[node distance = 2cm, auto]
    ...
    \node [block, right of=deploy, node distance=8em] (srvs) {Configuração dos nós};
    ...
    \node [block, below of=wan, node distance=10em] (run) {Execução de um experimento};

    \node [decision, right of=run, node distance=10em] (loop) {Último experimento?};
    
    \node [decision, above of=loop, node distance=10em] (reconf) {Modo ou
localidade mudaram?};

    \node [block, right of=loop, node distance=10em] (stop) {Término do Riak};

    \node [block, right of=stop, node distance=8em] (end) {Fim};
    ...
    \path [line] (loop) -- node [near start] {sim}(stop);

    \path [line] (reconf) -- node [near start] {não} (wan);

    \path [line] (reconf) |- node [near start] {sim}(riak);
...
\label{fig:fluxo_de_trabalho_da_execucao_do_estudo}
\end{figure}
